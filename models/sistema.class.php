...
class Sistema
{
    ...
    */
    ...
    public function cargarPDF($dimension, $destino)
    {
        if ($_FILES[$dimension]['error'] == 0) {
            $tiposPermitidos = array("application/pdf", "application/pdf", "image/png");
            if (in_array($_FILES[$dimension]['type'], $tiposPermitidos)) {
                if ($_FILES[$dimension]['size'] <= 512000) {
                    $nombre = md5(time());
                    $extension = explode("/", $_FILES[$dimension]['type']);
    ...
    public function validarUsuario($id_usuario){
        $this->connect();
        $tipoUsuario = $this->loginUsuario($id_usuario);
        return $tipoUsuario;
    }

    public function loginUsuario($id_usuario){
        $this->connect();
        $sql = "SELECT * FROM usuario_rol where id_usuario=:id_usuario";
            $stmt = $this->con->prepare($sql);
    ...
    public function validarRol($rol)
    {
        ...
        if (!in_array($rol, $roles)) {
           // require_once('../views/headerinicio.php');
